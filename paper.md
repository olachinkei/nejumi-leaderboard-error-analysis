...
それでもフロンティアモデルが間違える問題は残っており、能力不足だけでなく、問題設計自体の曖昧さや、採点プロトコル・評価器（judge）側の実装・版の違いに起因するものも含まれる。著名なベンチマークを人手で再検証すると一定数の誤りが見つかるという報告[2]や、評価器の信頼性そのものを問う研究群[3]があり、本研究が見つけた採点ロジックの不具合や評価バージョンの混在もこの系統に位置づけられる。なお、学習データへのベンチマーク混入や、非公開の複数バリアントを試し最良の結果だけを公開するといったリーダーボード運用上の公平性の問題は対象外とする。
...
ここまで見たように、ベンチマークには様々な問題があり、深掘りをして初めて見えてくるものが多い。一方、そうした問題が見つかるたびにベンチマークを更新し、比較対象となる既存モデルもすべて再評価するのは容易ではない。Nejumi LLM Leaderboardはおおよそ100個のモデルを評価対象とし、更新のたびに全モデルを再評価する必要があるため、リーダーボードの鮮度を保つことには構造的な難しさがある。次々と明らかになる問題点を追いきることは難しいが、少なくとも自分たちが利用するベンチマークについては性質を詳しく理解した上でモデル選定に用いることが重要になる。

リーダーボードを更新する際には、こうした問題への対応をできるだけ行う必要があるが、提案手法をそのまま利用すべきかはベンチマークの正しさ以外の観点からも考えた方が良い。本題から少し逸脱するが、コスト面についても紹介する。

例えば、SWE-benchのように複数ステップの試行錯誤を繰り返すエージェント型のタスクでは、出力データ自体は小さくても、token使用量や実行時間はごく一部のtask実行に強く集中する傾向がある。モデルの出力自体は早い段階でほぼ決着していても、その後の正誤確認や再試行が長く続くことでコストが積み重なる。実際、SWE-bench Proでは約5%のタスクが40%以上のコストを占めていた。性能とコストのバランスを鑑みれば、こうしたタスクは除外するなどの工夫が求められ、評価ベンチマーク作成者も極度にコストのかかるタスクには適切なデフォルトパラメータを設定するなどの工夫をするべきだろう。

特定のスナップショットではベンチマークに問題が含まれ、完全な評価体系の構築は困難だが、過去の評価上の問題は長い目では解決されつつある。本記事が評価業界の改善の一助となれば幸いである。
